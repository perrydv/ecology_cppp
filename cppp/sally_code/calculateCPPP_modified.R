...
calcDiscrepancies <- nimbleFunction(
    setup = function(model,
        ...
        for(i in seq_along(discrepancyFunctions))
            discrepancyFunctionList[[i]] <- discrepancyFunctions[[i]](discrepancyFunctionsArgs[[i]])
    ...
    run = function(MCMCOutput = double(2)){
        ...
        for(i in 1:nSamples){
            ...
            for(j in 1:nDiscrepancyFuns) {
                obsDiscrepancy <- discrepancyFunctionList[[j]]$run(model)
            ...
            for(j in 1:nDiscrepancyFuns) {
                simDiscrepancy <- discrepancyFunctionList[[j]]$run(model)
