...
# create df to store p values conditioned on latent states
p_values_basic <- matrix(NA, nrow = n_datasets, ncol = 4)
colnames(p_values_basic) <- c("deviance", "chi_sq", "lik_ratio", "ftukey")

# create df to store p values not conditioned on latent states
p_values_basic_latent <- matrix(NA, nrow = n_datasets, ncol = 4)
colnames(p_values_basic_latent) <- c("deviance", "chi_sq", 
                                     "lik_ratio", "ftukey")
...
for (i in 1:n_datasets) {
  ...
  # calculate p values, conditioned on latent states
  ...
  # calculate p values, not conditioned on latent states
  p_values_basic_latent[i, "deviance"] <- mean(
    samples[, "D_rep_latent_total"] > samples[, "D_obs_total"]
  )
  p_values_basic_latent[i, "chi_sq"] <- mean(
    samples[, "chi_rep_latent_total"] > samples[, "chi_obs_total"]
  )
  p_values_basic_latent[i, "lik_ratio"] <- mean(
    samples[, "ratio_rep_latent_total"] > samples[, "ratio_obs_total"]
  )
  p_values_basic_latent[i, "ftukey"] <- mean(
    samples[, "tukey_rep_latent_total"] > samples[, "tukey_obs_total"]
  )
  
}
saveRDS(p_values_basic, "pvalues/null/basic.rds")
saveRDS(p_values_basic_latent, "pvalues/null/basic_latent.rds")
...
# create df to store p values not conditioned on latent states
p_values_cov_occ_latent <- matrix(NA, nrow = n_datasets, ncol = 4)
colnames(p_values_cov_occ_latent) <- c("deviance", "chi_sq", 
                                       "lik_ratio", "ftukey")
...
for (i in 1:n_datasets) {
  ...
  # calculate p values, not conditioned on latent states
  p_values_cov_occ_latent[i, "deviance"] <- mean(
    samples[, "D_rep_latent_total"] > samples[, "D_obs_total"]
  )
  p_values_cov_occ_latent[i, "chi_sq"] <- mean(
    samples[, "chi_rep_latent_total"] > samples[, "chi_obs_total"]
  )
  p_values_cov_occ_latent[i, "lik_ratio"] <- mean(
    samples[, "ratio_rep_latent_total"] > samples[, "ratio_obs_total"]
  )
  p_values_cov_occ_latent[i, "ftukey"] <- mean(
    samples[, "tukey_rep_latent_total"] > samples[, "tukey_obs_total"]
  )
  
}
saveRDS(p_values_cov_occ, "pvalues/null/cov_occ.rds")
saveRDS(p_values_cov_occ_latent, "pvalues/null/cov_occ_latent.rds")
...
# create df to store p values not conditioned on latent states
p_values_cov_occ_det_latent <- matrix(NA, nrow = n_datasets, ncol = 4)
colnames(p_values_cov_occ_det_latent) <- c("deviance", "chi_sq", 
                                           "lik_ratio", "ftukey")
...
for (i in 1:n_datasets) {
  ...
  # calculate p values, not conditioned on latent states
  p_values_cov_occ_det_latent[i, "deviance"] <- mean(
    samples[, "D_rep_latent_total"] > samples[, "D_obs_total"]
  )
  p_values_cov_occ_det_latent[i, "chi_sq"] <- mean(
    samples[, "chi_rep_latent_total"] > samples[, "chi_obs_total"]
  )
  p_values_cov_occ_det_latent[i, "lik_ratio"] <- mean(
    samples[, "ratio_rep_latent_total"] > samples[, "ratio_obs_total"]
  )
  p_values_cov_occ_det_latent[i, "ftukey"] <- mean(
    samples[, "tukey_rep_latent_total"] > samples[, "tukey_obs_total"]
  )
  
}
saveRDS(p_values_cov_occ_det, "pvalues/null/cov_occ_det.rds")
saveRDS(p_values_cov_occ_det_latent, "pvalues/null/cov_occ_det_latent.rds")
...
# create df to store p values not conditioned on latent states
p_values_sp_ranef_latent <- matrix(NA, nrow = n_datasets, ncol = 4)
colnames(p_values_sp_ranef_latent) <- c("deviance", "chi_sq", 
                                        "lik_ratio", "ftukey")
...
for (i in 1:n_datasets) {
  ...
  # calculate p values, not conditioned on latent states
  p_values_sp_ranef_latent[i, "deviance"] <- mean(
    samples[, "D_rep_latent_total"] > samples[, "D_obs_total"]
  )
  p_values_sp_ranef_latent[i, "chi_sq"] <- mean(
    samples[, "chi_rep_latent_total"] > samples[, "chi_obs_total"]
  )
  p_values_sp_ranef_latent[i, "lik_ratio"] <- mean(
    samples[, "ratio_rep_latent_total"] > samples[, "ratio_obs_total"]
  )
  p_values_sp_ranef_latent[i, "ftukey"] <- mean(
    samples[, "tukey_rep_latent_total"] > samples[, "tukey_obs_total"]
  )
  
}
saveRDS(p_values_sp_ranef, "pvalues/null/sp_ranef.rds")
saveRDS(p_values_sp_ranef_latent, "pvalues/null/sp_ranef_latent.rds")
