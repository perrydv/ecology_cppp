...
# 
#     # simulate X
#     simulated_x[n, i, , ] <- cbind(rep(1, nSites), rep(rnorm(nSites / 4), 4), 
#                                    rnorm(nSites))
# 
#     # simulate y
...
# simulate data
simulated_y <- array(NA, dim = c(nDatasets, length(exp), nSites, nVisits))
simulated_x <- array(NA, dim = c(nDatasets, length(exp), nSites,
                                 length(beta)))
...
for (n in seq_along(1:nDatasets)) {
  for (i in seq_along(exp)) {
    ...
    simulated_y[n, i, , ] <- simulate_cov_occ_nonlin(
      params = list(p = p, beta = beta, exp = exp[i]),
      simulated_x[n, i, , ], nSites, nVisits
    )
  }
}
...
discrepancyFunctions <- list(chisqDiscFunction_sitecov)
discrepancyNames <- c("Chi-Square")
args <- list(y = "y", x_site = "x_site", nVisits = nVisits,
             beta = "beta", latent_occ = "z")
